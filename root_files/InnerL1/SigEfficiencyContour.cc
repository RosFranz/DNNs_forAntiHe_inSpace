...
void SaveAll(TString &selection, TFile *f_out);
...
void SigEfficiencyContour(bool MCorData=true, TString CLtrainOnMCFile="000", TString CLvalOnISSFile="000", TString AEtrainOnMCFile="000", TString AEvalOnISSFile="000") {
    ...
    const bool isLocal = true; //True for debugging
    ...
    for(int i=0; i<nRigbins_ams02He-1; ++i){
        ...
        to_draw = "scores_pos:anomaly_score>>+"+ScoresNameISS.at(i);
    ...
    for(int i=0; i<nRigbins_ams02He-1; ++i){
        ...
        SelScores.at(i) = AglSel +" && abs(Rinner)>"+TString(ossLow.str())+" && abs(Rinner)<"+TString(ossHigh.str());
        if(useSafetyFactor) SelScores.at(i) += isAbIGRFpos;
        if(useCutOff) SelScores.at(i) += IGRFneg;
    ...
    for(int i=0; i<nRigbins_ams02He-1; ++i) {
        ...
        to_draw = "scores_pos:anomaly_score>>+"+ScoresName.at(i);
    ...
    for(int i=0; i<nRigbins_ams02He-1; ++i){
        ...
        to_draw = "scores_neg:NEGanomaly_score>>+"+ScoresNameNeg.at(i);
    ...
    for(int i=0; i<nRigbins_ams02He-1; ++i){
        ...
        //taking into account the prescaling (without GM cutoff) and R<200 *0.3 
        (isMC) ? h1Mass2ISS.at(i)->Scale(2.5e+2) : h1Mass2ISS.at(i)->Scale(2.5e+2/pow(0.3,2)); 
        ...
        (isMC) ? h1Mass2Pos.at(i)->Scale(1.e+2/0.3) : h1Mass2Pos.at(i)->Scale(1.e+2);
