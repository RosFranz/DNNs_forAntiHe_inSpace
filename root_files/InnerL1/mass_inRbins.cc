...
void mass_inRbins(bool MCorData=true, TString CLtrainOnMCFile="000", TString CLvalOnISSFile="000", TString AEtrainOnMCFile="000", TString AEvalOnISSFile="000"){
    ...
    TString to_draw;
    array<TString, TotRig> draw_MCpos_CSinvMass{"scores_pos:1./((2.*abs("+NameR.at(0)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))",
                                                "scores_pos:1./((2.*abs("+NameR.at(1)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))"};
    array<TString, TotRig> draw_MCneg_CSinvMass{"scores_neg:1./((2.*abs("+NameR.at(0)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))",
                                                "scores_neg:1./((2.*abs("+NameR.at(1)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))"};
    array<TString, TotRig> draw_ISSpos_CSinvMass{"ISS_pos.scores_pos:1./((2.*abs("+NameR.at(0)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))",
                                                 "ISS_pos.scores_pos:1./((2.*abs("+NameR.at(1)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))"};
    array<TString, TotRig> draw_ISSneg_CSinvMass{"ISS_neg.scores_neg:1./((2.*abs("+NameR.at(0)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))",
                                                 "ISS_neg.scores_neg:1./((2.*abs("+NameR.at(1)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))"};
    ...
    array<TString, TotRig> draw_MCneg_ASinvMass{"MCneg.NEGanomaly_score:1./((2.*abs("+NameR.at(0)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))",
                                                 "MCneg.NEGanomaly_score:1./((2.*abs("+NameR.at(1)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))"};
    ...
    array<TString, TotRig> draw_ISSneg_ASinvMass{"ISSneg.NEGanomaly_score:1./((2.*abs("+NameR.at(0)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))",
                                                 "ISSneg.NEGanomaly_score:1./((2.*abs("+NameR.at(1)+")*sqrt(1-pow(beta_rich,2)))/(beta_rich*0.9315))"};
    ...
    //                "-Rinner<100 && abs(SigmaUpLow)<0.5 && ACC_AntiCounter==0 && NEGanomaly_score<=0.9 && scores_neg>0.1";
    ...
    TotSelection = "hasRich==1 && beta_rich>0.953 && beta_rich < 0.999 && isNaF==0 && hasGoodImpact==1 && kprob_rich>0.01 &&"
                   "1<charge2_rich && charge2_rich<4 && ringPMTs2_rich>5 && (measPE_Corr_rich/totPE_Uncorr_rich)>0.4 &&"
                   "scores_neg>=0.3 && abs(SigmaUpLow)<=0.4 && NEGanomaly_score<=0.8 && abs(IGRFneg)<abs(Rinner)";
    printf("Selection for survivors: %s\n", TotSelection.Data());
