...
void trainOnISS_mass(TString CLdataFile="000", TString CLmcFile="000", TString AEdataFile="000", TString AEmcFile="000"){
    ...
    for(int i=0; i<aeCutValues.size(); ++i){
        ...
        aeAglISSnegNumSel.push_back("&& ISSneg.anomaly_score<"+to_string(aeCutValues.at(i)));
        aeAglISSnegDenomSel.push_back("&& ISSneg.anomaly_score<"+to_string(aeCutValues.at(i)));
        ...
        aeNaFISSnegNumSel.push_back("&& ISSneg.anomaly_score<"+to_string(aeCutValues.at(i)));
        aeNaFISSnegDenomSel.push_back("&& ISSneg.anomaly_score<"+to_string(aeCutValues.at(i)));
        if(useSafetyFactor) {
            //ae
    ...
    for(int i=0; i<clCutValues.size(); ++i){
        //classifier
        clAglISSposNumSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clAglISSposDenomSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clAglISSnegNumSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clAglISSnegDenomSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clAglMCposNumSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clAglMCposDenomSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clAglMCnegNumSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clAglMCnegDenomSel.push_back("&& scores>"+to_string(clCutValues.at(i)));

        clNaFISSposNumSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clNaFISSposDenomSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clNaFISSnegNumSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clNaFISSnegDenomSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clNaFMCposNumSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clNaFMCposDenomSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clNaFMCnegNumSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
        clNaFMCnegDenomSel.push_back("&& scores>"+to_string(clCutValues.at(i)));
    ...
    for(int i=0; i<aeCutValues.size(); ++i){
        for(int j=0; j<clCutValues.size(); ++j){
            combAglISSposNumSel.push_back("&& ISS.anomaly_score<"+to_string(aeCutValues.at(i))+" && scores>"+to_string(clCutValues.at(j)));
            combAglISSposDenomSel.push_back("&& ISS.anomaly_score<"+to_string(aeCutValues.at(i))+" && scores>"+to_string(clCutValues.at(j)));
            combAglISSnegNumSel.push_back("&& ISSneg.anomaly_score<"+to_string(aeCutValues.at(i))+" && scores>"+to_string(clCutValues.at(j)));
            combAglISSnegDenomSel.push_back("&& ISSneg.anomaly_score<"+to_string(aeCutValues.at(i))+"&& scores>"+to_string(clCutValues.at(j)));

            combNaFISSposNumSel.push_back("&& ISS.anomaly_score<"+to_string(aeCutValues.at(i))+" && scores>"+to_string(clCutValues.at(j)));
            combNaFISSposDenomSel.push_back("&& ISS.anomaly_score<"+to_string(aeCutValues.at(i))+" && scores>"+to_string(clCutValues.at(j)));
            combNaFISSnegNumSel.push_back("&& ISSneg.anomaly_score<"+to_string(aeCutValues.at(i))+" && scores>"+to_string(clCutValues.at(j)));
            combNaFISSnegDenomSel.push_back("&& ISSneg.anomaly_score<"+to_string(aeCutValues.at(i))+" && scores>"+to_string(clCutValues.at(j)));
    ...
    for(int i_tree=0; i_tree<2; i_tree++){ //loop on CLtrees
        ...
        for(int j_rig=0; j_rig<TotRig; j_rig++){ //loop on rig
            if(i_tree%2==0) { // Only positives
                ...
                drawclRCSMC.push_back("scores:log10(abs("+NameR.at(j_rig)+"))>>+"+nRCSMC);
                ...
                drawclRCS.push_back("scores:log10(abs("+NameR.at(j_rig)+"))>>+"+nRCS);
                drawaeRAS.push_back("ISSneg.anomaly_score:log10(abs("+NameR.at(j_rig)+"))>>+"+nRAS);
                drawclRCSMC.push_back("scores:log10(abs("+NameR.at(j_rig)+"))>>+"+nRCSMC);
                drawaeRASMC.push_back("anomaly_score:log10(abs("+NameR.at(j_rig)+"))>>+"+nRASMC);
    ...
    TotSelection = "hasRich==1 && beta_rich>0.953 && beta_rich < 0.999 && isNaF==0 && hasGoodImpact==1 && kprob_rich>0.01 &&"
                    "1<charge2_rich && charge2_rich<4 && ringPMTs2_rich>5 && (measPE_Corr_rich/totPE_Uncorr_rich)>0.4 &&"
                    "scores>=0.3 && abs(SigmaUpLow)<=0.4 && anomaly_score<=0.8 && isAbIGRFneg";
    printf("Selection for survivors: %s\n", TotSelection.Data());
