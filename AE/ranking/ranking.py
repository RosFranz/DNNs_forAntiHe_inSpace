...
#=====> Input arguments
parser = argparse.ArgumentParser(description="AE ranking script")
...
columns_to_keep = ['ACC_AntiCounter',
                    ...
                    'SigmaInnerL1',          'SigmaUpLow',            'Rinner']
...
print('Number of entries in the positive dic:', UnNormData.shape[0])
...
UnNormDataNeg = MyH5man.ReadH5toDataFrame()
UnNormDataNeg.drop(columns=[col for col in UnNormDataNeg.columns if col not in columns_to_keep], inplace=True)
gc.collect()
print('I am cutting negative MC in REC RIG below MDR = 200 GV')
UnNormDataNeg = UnNormDataNeg[UnNormDataNeg['Rinner']>-200]
UnNormDataNeg.drop(columns=['Rinner'], inplace=True)
UnNormDataNeg = UnNormDataNeg.iloc[np.random.permutation(len(UnNormDataNeg))]
gc.collect()
print('Current columns NEGATIVES:', UnNormDataNeg.columns, 'Total columns:', len(UnNormDataNeg.columns))
print('Number of entries in the negative dic:', UnNormDataNeg.shape[0])
NegColumns = UnNormDataNeg.columns.tolist()

#=====> Labels
label =np.ones(UnNormData.shape[0])
label =np.hstack((label, np.zeros(UnNormDataNeg.shape[0]))).T
...
Xbkg=UnNormDataNeg.values
Xsig=UnNormData[:len(lbkg)].values

X=np.vstack((Xsig,Xbkg))
label=np.hstack((lsig, lbkg))
del lsig, lbkg, Xsig, Xbkg
del UnNormData, UnNormDataNeg
gc.collect
...
for train_index, test_index in kf.split(X, label):
    ...
    if not g.TRAIN_ON_ISS:
        ...
        X_cv_W = np.array(X_cv_W, dtype=np.float64)
        x_test_cv_W = np.array(x_test_cv_W, dtype=np.float64)
    
    print('Type: ', type(X_cv) ,'data-type: ', X_cv.dtype)
    X_cv = np.array(X_cv, dtype=np.float64)
    x_test_cv = np.array(x_test_cv, dtype=np.float64)
    print('Type: ', type(X_cv) ,'data-type: ', X_cv.dtype)
...
plt.grid(which='both')
plt.gca().xaxis.set_minor_locator(ticker.LogLocator(base=10, subs='all'))  # Set minor locator with all subs along the x-axis
...
if(target_like):
    if g.TRAIN_ON_ISS:
        plt.savefig('ISS_BW_feature_ranking_trgtLike.png', dpi=300)  
    else:
        plt.savefig('MC_BW_feature_ranking_trgtLike.png', dpi=300)  
if((not random) and (not target_like)):
    if g.TRAIN_ON_ISS:
        plt.savefig('iSS_BW_feature_ranking.png', dpi=300)  
    else:
        plt.savefig('MC_BW_feature_ranking.png', dpi=300)  
...
if((not random) and (not target_like)):
    if g.TRAIN_ON_ISS:
        plt.savefig('ISS_Color_feature_ranking.png', dpi=300)
    else:
        plt.savefig('MC_Color_feature_ranking.png', dpi=300)
...
if g.TRAIN_ON_ISS:
    b.to_csv('ISS_ranking.csv')
else:
    b.to_csv('MC_ranking.csv')
print("Done!")
