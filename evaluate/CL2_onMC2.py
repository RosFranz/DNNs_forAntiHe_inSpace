...
#=====> Input arguments
parser = argparse.ArgumentParser(description="CL on MC2 testing script")
...
print('Loading CC He4 MC dataset')
#INPUT
if g.IS_OLD_RESULT:
...
#OUTPUT
rOUTpath = g.NEW_FOLDER_PATH + now+'_trainOnISS_2CL_MC_scores2.root'
#DEBUG
if g.DEBUG:
...
MCneg = MyH5man.ReadH5toDataFrame()
MCneg['orig_idx'] = np.arange(len(MCneg), dtype=np.int64)
g_CL2.CL2_COLUMNS.append('orig_idx')
label_neg = MCneg['RigLabel'].values
MCneg.drop(columns=[col for col in MCneg.columns if col not in g_CL2.CL2_COLUMNS], inplace=True)
print('Input features: ', MCneg.columns)  
test_idx = MCneg['orig_idx'].values
MCneg.drop(columns=['orig_idx'], inplace=True)
X=MCneg.values
del MCneg
gc.collect()
