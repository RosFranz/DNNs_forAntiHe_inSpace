...
#=====> Input arguments
parser = argparse.ArgumentParser(description="CL on MC3 testing script")
...
#OUTPUT
rOUTpath = g.NEW_FOLDER_PATH + now+'_trainOnISS_2CL_MC_scores3.root'
#DEBUG
if g.DEBUG:
    ...
    MyH5man.SetHe3h5Name("He3CL_21000.h5")
    MyH5man.SetUseHe3(True)

MCpos = MyH5man.ReadH5toDataFrame()
MCpos['orig_idx'] = np.arange(len(MCpos), dtype=np.int64)
g_CL2.CL2_COLUMNS.append('orig_idx')
label_pos = MCpos['RigLabel'].values
MCpos.drop(columns=[col for col in MCpos.columns if col not in g_CL2.CL2_COLUMNS], inplace=True)
print('Input features: ', MCpos.columns)  
test_idx = MCpos['orig_idx'].values
MCpos.drop(columns=['orig_idx'], inplace=True)
X=MCpos.values
del MCpos
gc.collect()

test_sample  = scaler.transform(X)
test_dataset = CL2.myDataset(test_sample,test_idx,label_pos)
...
for i, (sample,te_idx,MyLabels) in enumerate(test_loader):
    ...
    out = out.squeeze(-1)
    dic['scores'].append(out.cpu().detach().numpy())
