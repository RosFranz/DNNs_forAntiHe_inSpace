...
if(g.TRAIN_ON_ISS):
    ...
    #OUTPUT
    rOUTpath = g.NEW_FOLDER_PATH + now+'_trainOnISS_2CL_ISS_scores2.root'
    #DEBUG
    if g.DEBUG:
    ...
    if(g_CL2.CL2_LOSSWEIGHTS):
        rOUTpath = g.NEW_FOLDER_PATH + now+'_trainOnMC_withW_2CL_MC_scores2.root'
    else:
        rOUTpath = g.NEW_FOLDER_PATH + now+'_trainOnMC_2CL_MC_scores2.root'
    #DEBUG
    if g.DEBUG:
        MyH5man.SetInPath(in_path=INpath)
        MyH5man.SetMaxRows(max_rows=100000)
...
NegFrame = MyH5man.ReadH5toDataFrame()
NegFrame['orig_idx'] = np.arange(len(NegFrame), dtype=np.int64)
g_CL2.CL2_COLUMNS.append('orig_idx')
label_neg = NegFrame['RigLabel'].values
NegFrame.drop(columns=[col for col in NegFrame.columns if col not in g_CL2.CL2_COLUMNS], inplace=True)
print('Input features: ', NegFrame.columns)  
test_idx = NegFrame['orig_idx'].values
NegFrame.drop(columns=['orig_idx'], inplace=True)
if not g.TRAIN_ON_ISS:
    NegFrame.drop(columns=['weight'], inplace=True)
X=NegFrame.values
del NegFrame
gc.collect()
...
for i, (sample,te_idx,MyLabels) in enumerate(test_loader):
    out = model(sample.float().to(device))
    out = out.squeeze(-1)
