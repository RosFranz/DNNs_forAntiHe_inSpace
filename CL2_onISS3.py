...
#=====> Input arguments
parser = argparse.ArgumentParser(description="CL on ISS3 testing script")
...
if(g_CL2.CL2_LOSSWEIGHTS):
    rOUTpath = g.NEW_FOLDER_PATH + now+'_trainOnMC_withW_2CL_ISS_scores3.root'
else:
    rOUTpath = g.NEW_FOLDER_PATH + now+'_trainOnMC_2CL_ISS_scores3.root'
#DEBUG
if g.DEBUG:
    ...
    MyH5man.SetInPath(in_path=INpath)
ISSpos = MyH5man.ReadH5toDataFrame()
ISSpos['orig_idx'] = np.arange(len(ISSpos), dtype=np.int64)
g_CL2.CL2_COLUMNS.append('orig_idx')
label_pos = ISSpos['RigLabel'].values
ISSpos.drop(columns=[col for col in ISSpos.columns if col not in g_CL2.CL2_COLUMNS], inplace=True)
print('Input features: ', ISSpos.columns)  
test_idx = ISSpos['orig_idx'].values
ISSpos.drop(columns=['orig_idx'], inplace=True)
X=ISSpos.values
del ISSpos
gc.collect()
...
dic['test_idx']=[]
model.eval()
print('Processing negative features')
